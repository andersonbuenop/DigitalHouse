package aula02;

import java.util.Scanner;

public class Main {

    public static void main(String[] args) {
        // exercicio 00
     /*   int num1 = 6;
        int num2 = 2;
        int result = num1%num2;

        if(result == 0){
            result = num1 / num2;
            System.out.println(num1+" é divisivel por "+num2+" e o resultado é "+result);
        }else{
            System.out.println(num1+" não é divisivel exato por "+num2);
        }*/

        //exercicio 01
        /*String saudacao = "hello world";
        String grettings = "hello world";
        if (saudacao.equals(grettings)) {
            System.out.println("Textos iguais uhuuull!");
        } else {
            System.out.println("Deu ruim, textos não são iguais!");
        }*/

        //ecercicio 02
        /*int num1 = 9;
        int num2 = 6;

        if(num1>num2){
            System.out.println(num1+" é maior que "+num2);
        }else if(num1 == num2){
            System.out.println(num1+" é igual a "+num2);
        }else{
            System.out.println(num1+" é menor que "+num2);
        }*/

        //exercicio 03
        /*String name, lastName, initialName, initialLastName;
        Scanner scanner;
        scanner = new Scanner(System.in);

        System.out.println("Entre com um nome: ");
        name = scanner.next();
        System.out.println("Entre com um sobrenome: ");
        lastName = scanner.next();

        System.out.println("Seja bem vindo: "+name+" "+lastName);

        initialName = name.substring(0,1);
        initialLastName = lastName.substring(0,1);
        System.out.println("Seu email será : "+initialName.toLowerCase()+initialLastName.toLowerCase()+"@company.com\nBom trabalho!");
*/
/*
        boolean divisivel(int num1, int num2){
            if (num1%num2==0){
                return true;
            }
            Scanner scanner;
            scanner = new Scanner(System.in);
            System.out.println("Entre com um numero :");
            num1 = scanner.nextInt();
            System.out.println("Entre com o segundo numero: ");
            num2 = scanner.nextInt();

            System.out.println(divisivel(num1, num2));
        }*/

        Scanner scanner;
        scanner = new Scanner(System.in);
        System.out.println("Nome: ");
        String nome = scanner.next();
        System.out.println("Sobrenome: ");
        String sobrenome = scanner.next();
        System.out.println("Dia: ");
        String dia = scanner.next();
        System.out.println("Mês: ");
        String mes = scanner.next();
        System.out.println("Ano: ");
        String ano = scanner.next();

        System.out.println("Seu nome é: "+nome+" "+sobrenome+"\ne você nasceu em: "+dia+"/"+mes+"/"+ano);
    }
}